...
"""Compare the four prompt surfaces used by rwkv-skills over long contexts."""
...
ANSWERS = {"early": "EARLY-RWKV-314159", "late": "LATE-RWKV-271828"}

FORMAT_SUFFIXES = {
    "colon_cot": "\n\nAssistant: <think",
    "flower_cot": "✿Bot✿<think",
...
def _task_header(format_name: str) -> str:
    if format_name == "flower_cot":
...
def _build_prompt(
    ...
    def make(repeats: int) -> str:
        ...
        if placement == "early":
            context = (
                f"\nThe exact answer token is {answer}.\n"
        ...
        if format_name == "flower_cot":
            question = "✿Question: What is the exact answer token?"
        else:
            question = "Question: What is the exact answer token?"
        return (
            _task_header(format_name)
            + context
            + question
            + FORMAT_SUFFIXES[format_name]
        )
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for format_name in FORMAT_SUFFIXES:
